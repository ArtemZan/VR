...
namespace VR
{
	...
	/* void Mesh2DContainer::Line(float length, float width, float border_radius, size_t border_sections)
	{
	...
	void Mesh2DContainer::Line(const math::vec2& start, const math::vec2& end, float width, float border_radius, size_t border_sections)
	{
	...
	bool Mesh2DContainer::IsHovered(const math::mat3& view) const
	{
		...
		const size_t vertexSize = GetGeometry().GetVertexSize();
		const size_t vertCount = GetGeometry().m_data->GetVerticesSize() / vertexSize;
		const size_t posOffset = GetGeometry().GetPosOffset();

		for (int i = 0; i < GetGeometry().m_data->GetIndicesCount() - 2; i += 3)
		{
			const math::vec2 p1 = GetGeometry().GetPos(GetGeometry().m_data->GetIndex(i));
			const math::vec2 p2 = GetGeometry().GetPos(GetGeometry().m_data->GetIndex(i + 1));
			const math::vec2 p3 = GetGeometry().GetPos(GetGeometry().m_data->GetIndex(i + 2));
